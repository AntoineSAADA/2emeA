...
def decoupe(message, long):
    """Fonction qui découpe des blocs de longueur long dans une chaine de caractères "message" et retourne une liste des blocs

    Args:
        message (String): Le message à coder
        long (int): la longueur à ne pas depasser 

    Returns:
        List: La liste des blocs de longueur long dans la chaine message
    """
    idec1 , idec2 = 0 , long #définition des indices de découpe
    l = [] #liste des blocs
    while idec2 <= len(message): #tant que idec2 est inférieur à la longueur de message
        l.append(message[idec1:idec2]) #on ajoute le bloc de message[idec1:idec2] à la liste l
        idec1 , idec2 = idec2 , idec2 + long #on incrémente idec1 et idec2 de long
    m = len(message%long) #on calcule le reste de la division de la longueur de message par long
    if m != 0: #si m est différent de 0
        l.append(message[len(message)-m:]) #on ajoute le dernier bloc de message à la liste l
    return l #fonction qui chiffre le message
...
def key():
    ...
    p = np.random.choice(1000,1) #choix d'un nombre premier entre 0 et 1000
    q = np.random.choice(1000,1) #choix d'un nombre premier entre 0 et 1000

    while not est_premier(p):
        p = np.random.choice(1000,1) #tant que p n'est pas premier on recommence
        
    while not est_premier(q):
        q = np.random.choice(1000,1) #tant que q n'est pas premier on recommence
